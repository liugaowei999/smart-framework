...
/**
 * 依赖注入 -- 助手类 对通过@Inject注解的属性，通过框架自动实例化。 实例化的过程不是开发者通过new来实例化对象，
 * 而是通过框架自身来实例化，像这类实例化过程 称为IoC（Inversion of Control）控制反转。也称为DI（Dependency
 * Injection）依赖注入。
 * 
 * @author liugaowei
 *
