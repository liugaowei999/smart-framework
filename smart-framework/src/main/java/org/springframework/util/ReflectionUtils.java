...
public abstract class ReflectionUtils {
	...
	public static Field findField(Class<?> clazz, String name, Class<?> type) {
		...
		while (!Object.class.equals(searchType) && searchType != null) {
			...
			searchType = searchType.getSuperclass();// 如果当前类没有找到， 则到上一级父类查找这个字段是否存在
		}
		return null;
	}
	 ...
	 * semantics, the returned value is automatically wrapped if the underlying
	 * field
	...
	public static Method findMethod(Class<?> clazz, String name, Class<?>... paramTypes) {
		...
		while (searchType != null) {
			/**
			 * getMethods() ---> 返回这个类或接口的所有公共的方法，包括继承到的父类或接口的方法；
			 * getDeclaredMethods() ---> 返回这个类或接口的所有的私有、公共、保护、默认的方法，但是不包括继承来得方法；
			 */
	 ...
	 * superclasses. Leaf
	 * class methods are included first and while traversing the superclass
	 * hierarchy any methods found
