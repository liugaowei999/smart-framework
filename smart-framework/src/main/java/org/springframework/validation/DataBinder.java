 ...
 *
 * <p>
 * Note that there are potential security implications in failing to set an
 * array
 * of allowed fields. In the case of HTTP form POST data for example, malicious
 * clients
 * can attempt to subvert an application by supplying values for fields or
 * properties
 * that do not exist on the form. In some cases this could lead to illegal data
 * being
 * set on command objects <i>or their nested objects</i>. For this reason, it is
 * <b>highly recommended to specify the {@link #setAllowedFields allowedFields}
 * property</b>
 * on the DataBinder.
 *
 * <p>
 * The binding results can be examined via the {@link BindingResult} interface,
 * extending the {@link Errors} interface: see the {@link #getBindingResult()}
 * method.
 ...
 * {@link BindingErrorProcessor}
 * strategy, processing for missing fields and property access exceptions: see
 * the
 * {@link #setBindingErrorProcessor} method. You can override the default
 * strategy
 * if needed, for example to generate different error codes.
 *
 * <p>
 * Custom validation errors can be added afterwards. You will typically want to
 * resolve
 * such error codes into proper user-visible error messages; this can be
 * achieved through
 * resolving each error via a {@link org.springframework.context.MessageSource},
 * which is
 ...
 * {@link MessageCodesResolver}
 * strategy: see the {@link #setMessageCodesResolver} method.
 * {@link DefaultMessageCodesResolver}'s
...
public class DataBinder implements PropertyEditorRegistry, TypeConverter {
	...
	/**
	 * Set whether this binder should attempt to "auto-grow" a nested path that
	 * contains a null value.
	 * <p>
	 * If "true", a null path location will be populated with a default object value
	 * and traversed
	 * instead of resulting in an exception. This flag also enables auto-growth of
	 * collection elements
	...
	/**
	 * Register fields that should <i>not</i> be allowed for binding. Default is
	 * none.
	 ...
	 * 
	 * @param pvs
	 *            property values to bind
	 * @see #doBind(org.springframework.beans.MutablePropertyValues)
	 */
	public void bind(PropertyValues pvs) {
		MutablePropertyValues mpvs = (pvs instanceof MutablePropertyValues) ? (MutablePropertyValues) pvs
	 ...
	 * 
	 * @param mpvs
	 *            the property values to be bound (can be modified)
	 ...
	 *            one or more hint objects to be passed to a {@link SmartValidator}
	 * @see #setValidator(Validator)
