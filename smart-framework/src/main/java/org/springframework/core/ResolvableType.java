...
public final class ResolvableType implements Serializable {
	...
	/**
	 * {@code ResolvableType} returned when no value is available. {@code NONE} is
	 * used
	 * in preference to {@code null} so that multiple method calls can be safely
	 * chained.
	 */
	...
	/**
	 * Late binding stored copy of the resolved value (valid when
	 ...
	 * 
	 * @param type
	 *            the underlying java type (may only be {@code null} for
	 *            {@link #NONE})
	 * @param variableResolver
	 *            the resolver used for {@link TypeVariable}s (may be {@code null})
	 * @param componentType
	 *            an option declared component type for arrays (may be {@code null})
	 */
	 ...
	 * {@link ResolvableType}
	 * was constructed. With the exception of the {@link #NONE} constant, this
	 * method will
	 * never return {@code null}. This method is primarily to provide access to
	 * additional
	 ...
	 * {@code type}. Attempts to follow the same rules as the Java compiler,
	 * considering
	...
	/**
	 * Return a {@link ResolvableType} for the specified nesting level. The nesting
	 * level
	 * refers to the specific generic parameter that should be returned. A nesting
	 * level
	 * of 1 indicates this type; 2 indicates the first nested generic; 3 the second;
	 * and so
	 ...
	 * generic
	 * for the given level. For example, an index of 0 would refer to a {@code Map}
	 * key;
	 * whereas, 1 would refer to the value. If the map does not contain a value for
	 * a
	 ...
	 * nested
	 * {@code List}. For convenience, if no indexes are specified the first generic
	 * is
	 * returned.
	 * <p>
	...
	/**
	 * Return an array of {@link ResolvableType}s representing the generic
	 * parameters of
	 * this type. If no generics are available an empty array is returned. If you
	 * need to
	 * access a specific generic consider using the {@link #getGeneric(int...)}
	 * method as
	 ...
	 * 
	 * @param fallback
	 *            the fallback class to use if resolution fails (may be
	 *            {@code null})
	 * @return an array of resolved generic parameters (the resulting array will
	 *         never be
	 *         {@code null}, but it may contain {@code null} elements})
	...
	/**
	 * Resolve this type to a {@link java.lang.Class}, returning {@code null} if the
	 * type
	 * cannot be resolved. This method will consider bounds of {@link TypeVariable}s
	 * and
	 ...
	 * {@code fallback} if the type cannot be resolved. This method will consider
	 * bounds
	 * of {@link TypeVariable}s and {@link WildcardType}s if direct resolution
	 * fails;
	...
	public int hashCode() {
		...
		hashCode = hashCode * 31 + ObjectUtils
				.nullSafeHashCode(this.variableResolver == null ? null : this.variableResolver.getSource());
	 ...
	 * 
	 * @param sourceClass
	 *            the source class (must not be {@code null}
	 ...
	 * Return a {@link ResolvableType} for the specified {@link Constructor}
	 * parameter
	 * with a given implementation. Use this variant when the class that declares
	 * the
	 ...
	 * Return a {@link ResolvableType} for the specified {@link Method} parameter
	 * with a
	 * given implementation. Use this variant when the class that declares the
	 * method
	...
	/**
	 * Return a {@link ResolvableType} for the specified {@link Type}. NOTE: The
	 * resulting
	 ...
	 * given
	 * owner type. NOTE: The resulting {@link ResolvableType} may not be
	 * {@link Serializable}.
	 * 
	 * @param type
	 *            the source type or {@code null}
	...
	/**
	 * Return a {@link ResolvableType} for the specified {@link Type} backed by a
	 * given
	...
	private static class WildcardBounds {
		 ...
		 * {@code null} if the specified type cannot be resolved to a
		 * {@link WildcardType}.
		 * 
		 * @param type
		 *            the source type
