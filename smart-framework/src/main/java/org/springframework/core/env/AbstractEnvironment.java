...
/**
 * Abstract base class for {@link Environment} implementations. Supports the
 * notion of
 * reserved default profile names and enables specifying active and default
 * profiles
 * through the {@link #ACTIVE_PROFILES_PROPERTY_NAME} and
 * {@link #DEFAULT_PROFILES_PROPERTY_NAME} properties.
 *
 * <p>
 * Concrete subclasses differ primarily on which {@link PropertySource} objects
 * they
 * add by default. {@code AbstractEnvironment} adds none. Subclasses should
 * contribute
 * property sources through the protected
 * {@link #customizePropertySources(MutablePropertySources)}
 * hook, while clients should customize using
...
public abstract class AbstractEnvironment implements ConfigurableEnvironment {
	...
	/**
	 * Name of property to set to specify active profiles: {@value}. Value may be
	 * comma
	 * delimited.
	 * <p>
	 * Note that certain shell environments such as Bash disallow the use of the
	 * period
	 * character in variable names. Assuming that Spring's
	 * {@link SystemEnvironmentPropertySource}
	...
	/**
	 * Name of property to set to specify profiles active by default: {@value}.
	 * Value may
	 * be comma delimited.
	 * <p>
	 * Note that certain shell environments such as Bash disallow the use of the
	 ...
	 * {@link #customizePropertySources(MutablePropertySources)} during construction
	 * to
	 * allow subclasses to contribute or manipulate {@link PropertySource} instances
	 * as
	 * appropriate.
	 * 
	...
	public AbstractEnvironment() {
		...
		if (this.logger.isDebugEnabled()) {
			this.logger.debug(format("Initialized %s with PropertySources %s", name, this.propertySources));
		}
	}
	 ...
	 * sources using {@link MutablePropertySources#addLast(PropertySource)} such
	 * that
	 ...
	 * </pre>
	 * 
	 * In this arrangement, properties will be resolved against sources A, B, C, D
	 * in that
	 * order. That is to say that property source "A" has precedence over property
	 * source
	 * "D". If the {@code Level2Environment} subclass wished to give property
	 * sources C
	 * and D higher precedence than A and B, it could simply call
	 * {@code super.customizePropertySources} after, rather than before adding its
	 * own:
	 * 
	 * <pre class="code">
	 * public class Level2Environment extends Level1Environment {
	 ...
	 * {@code remove}, or {@code replace} methods exposed by
	 * {@link MutablePropertySources}
	 * in order to create the exact arrangement of property sources desired.
	 *
	 * <p>
	 * The base implementation in
	 * {@link AbstractEnvironment#customizePropertySources}
	 * registers no property sources.
	 *
	 * <p>
	 * Note that clients of any {@link ConfigurableEnvironment} may further
	 * customize
	 * property sources via the {@link #getPropertySources()} accessor, typically
	 * within
	 ...
	 * Instance variables declared in subclasses and having default initial values
	 * should
	 ...
	 * callback is invoked by the {@link #AbstractEnvironment()} constructor, which
	 * may
	 * lead to a {@code NullPointerException} or other problems. If you need to
	 * access
	 * default values of instance variables, leave this method as a no-op and
	 * perform
	 * property source manipulation and instance variable access directly within the
	 * subclass constructor. Note that <em>assigning</em> values to instance
	 * variables is
	 * not problematic; it is only attempting to read default values that must be
	 * avoided.
	 *
	 ...
	 * is empty, check for the presence of the
	 * {@value #ACTIVE_PROFILES_PROPERTY_NAME}
	 ...
	 * {@link #setDefaultProfiles(String...)} or if the current set of default
	 * profiles
	 * consists only of {@linkplain #getReservedDefaultProfiles() reserved default
	 * profiles}, then check for the presence of the
	 * {@value #DEFAULT_PROFILES_PROPERTY_NAME} property and assign its value (if
	 * any)
				...
				protected String getSystemAttribute(String variableName) {
					try {
						...
						if (logger.isInfoEnabled()) {
							logger.info(format(
									"Caught AccessControlException when "
					...
					try {
						...
						if (logger.isInfoEnabled()) {
							logger.info(format("Caught AccessControlException when "
									...
									+ "returned [null]. Reason: %s", propertyName, ex.getMessage()));
						}
						return null;
					}
				}
	...
	public String toString() {
		return format("%s {activeProfiles=%s, defaultProfiles=%s, propertySources=%s}", getClass().getSimpleName(),
				this.activeProfiles, this.defaultProfiles, this.propertySources);
	}
