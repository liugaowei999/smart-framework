 ...
 *
 * <p>
 * Calls a static {@code valueOf(sourceType)} or Java 8 style
 * {@code of(sourceType)} method
 * on the target type to perform the conversion, if such a method exists.
 * Otherwise, it calls
 * the target type's constructor that accepts a single {@code sourceType}
 * argument, if such
 * a constructor exists. If neither strategy works, it throws a
...
final class ObjectToObjectConverter implements ConditionalGenericConverter {
	...
	public Object convert(Object source, TypeDescriptor sourceType, TypeDescriptor targetType) {
		...
		throw new IllegalStateException("No static valueOf(" + sourceClass.getName() + ") method or Constructor("
				+ sourceClass.getName() + ") exists on " + targetClass.getName());
	}

	static boolean hasValueOfMethodOrConstructor(Class<?> clazz, Class<?> sourceParameterType) {
		return getValueOfMethodOn(clazz, sourceParameterType) != null
				|| getConstructor(clazz, sourceParameterType) != null;
	}
