...
public abstract class ClassUtils {
	 ...
	 * 以基本类型的包装类型为key， 基本类型为value 的Map集合 Integer.class -> int.class.
	 */
	 ...
	 * 以基本类型为key， 基本类型的包装类型为value的Map集合 int.class -> Integer.class.
	 */
	 ...
	 * 以基本类型的字面值为key， 基本类型为value的Map集合 "int" -> "int.class".
	 */
	 ...
	 *             (that is, the class could not be found or the class file could
	 *             not be loaded)
	...
	public static Class<?> resolvePrimitiveClassName(String name) {
		...
		// int, boolean, char, double, float, byte, short long ----> 基本类型的名称长度都小于8； 长度判断纯粹是为了提高效率
	 ...
	 * the
	 * {@code GroovyObject} interface (for interface methods; on an implementation
	 * class,
	 * implementations of the {@code GroovyObject} methods will be marked as
	 * synthetic anyway).
	 * Note that, despite being synthetic, bridge methods
	 * ({@link Method#isBridge()}) are considered
	 * as user-level methods since they are eventually pointing to a user-declared
	 * generic method.
	 * 
	 * @param method
	 *            the method to check
	 ...
	 * @param classLoader
	 *            the ClassLoader to check against (may be {@code null},
